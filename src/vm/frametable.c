...
evict_page()
{
	...
	while(pagedir_is_accessed((frametable[clock_hand].owner)->pagedir, current_page)){
		if (current_page == 0xbffff000)
		{
			char * temp = (char *) frametable[clock_hand].page;
			/*int i;
			for (i = 3500; i < 4096; ++i)
	        {
	          printf("# [%c] (%x) <%x> \n", temp[i], temp[i], temp+i);
	        }
			printf("!!!!!!!!!!!!!! got stack page\n");*/
		}
	...
	bool page_isdirty = pagedir_is_dirty((frametable[clock_hand].owner)->pagedir, current_page);
	if(current_spinfo->instructions == STACK || page_isdirty)
	{
		current_spinfo->index_into_swap = move_into_swap(frametable[clock_hand].page, current_spinfo->instructions, page_isdirty);
		/*if (current_spinfo->instructions == STACK)
		{
			printf("************ %x\n", current_spinfo->upage_address);
		}*/
